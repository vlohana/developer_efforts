// src/internal/browser-buffer.js
var Buffer = class {
  /**
  ...
  static isBuffer(buffer) {
    return buffer != null && typeof buffer === "object" && buffer.string !== void 0;
  }
  /**
  ...
  static concat(buffers, length) {
    ...
    if (length !== void 0 && string.length > length) {
...
var EMPTY_STRING_BUFFER = Buffer.from("");
var SPACE_STRING_BUFFER = Buffer.from(" ");
var META_CHILD_CLOSE = Buffer.from(`<!--/lit-child-->`);
var META_CHILD_OPEN = Buffer.from(`<!--lit-child-->`);
var META_CLOSE = Buffer.from(`<!--/lit-->`);
var META_CLOSE_SHADOW = Buffer.from(`<!--/lit--></template>`);
...
function resolveNodeValue(value, tagName, withMetadata) {
  ...
  if (isDirectiveResult(value)) {
    ...
    if (directiveName === "unsafeHTML" || directiveName === "unsafeSVG") {
      ...
      return withMetadata ? [Buffer.from(`<!--lit-child ${digestForTemplateStrings(strings)}-->`), buffer, META_CHILD_CLOSE] : buffer;
    } else {
      value = result;
    }
  }
...
// src/internal/browser-digest.js
function digestForTemplateStrings2(strings) {
  const digestSize = 2;
  const hashes = new Uint32Array(digestSize).fill(5381);
  for (const s of strings) {
    for (let i = 0; i < s.length; i++) {
...
var Template = class {
   ...
   */
  constructor(strings) {
    this.digest = digestForTemplateStrings2(strings);
    this.strings = [];
    this.parts = [];
    this._parse(strings);
  }
...
// src/internal/browser-node-stream-template-renderer.js
